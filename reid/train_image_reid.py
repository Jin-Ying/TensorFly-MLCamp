...
def image_classification_val(loader, model, classifier, test_10crop=True):
    start_test = True
    with torch.no_grad():
        if test_10crop:
            ...
            for i in range(len(loader['val'][0])):
                ...
                outputs = []
                for j in range(10):
                    feature, _ = model(inputs[j])
                    feature,predict_out = classifier(feature)
...
def train(config):
    ...
    base_network = torch.load("./snapshot/finetune_pet/iter_01000_model.pth.tar")[0]
    base_network = base_network.cuda()
    classifier_network = network.Classifier(class_num=config["network"]["params"]["class_num"])
    classifier_network = classifier_network.cuda()

    parameter_list = classifier_network.get_parameters()
    ...
    for i in range(config["num_iterations"]):
        ...
        if i % config["test_interval"] == config["test_interval"] - 1:
            ...
            temp_acc, output, label, feature, logit = image_classification_val(dset_loaders, \
                base_network, classifier_network, test_10crop=prep_config["test_10crop"])
        ...
        if i % config["snapshot_interval"] == 0:
            torch.save(nn.Sequential(classifier_network), osp.join(config["output_path"], \
                "iter_{:05d}_classifier.pth.tar".format(i)))
        ...
        embed_source, outputs_source = classifier_network(features_source)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dset', type=str, default='aby', choices=['aby', 'bulldog', 'apbt', 'sbt', 'stanford-dog', 'mit', 'cub','car', 'pet'], help="The dataset or source dataset used")
    parser.add_argument('--s_dset_path', type=str, default='/data/finetune/pets_google/staffordshire_bull_terrier/train', help="The source dataset path list")
    parser.add_argument('--t_dset_path', type=str, default='/data/finetune/pets_google/staffordshire_bull_terrier/test', help="The target dataset path list")
    ...
    parser.add_argument('--output_dir', type=str, default='reid_aby_new', help="output directory of our model (in ../snapshot directory)")
    ...
    if config["dataset"] == "office":
        ...
        config["optimizer"]["lr_param"]["lr"] = 0.001 # optimal parameters
        config["network"]["params"]["class_num"] = 31
    elif config["dataset"] == "bulldog":
        config["optimizer"]["lr_param"]["lr"] = 0.001 # optimal parameters
        config["network"]["params"]["class_num"] = 64
    elif config["dataset"] == "apbt":
        config["optimizer"]["lr_param"]["lr"] = 0.001 # optimal parameters
        config["network"]["params"]["class_num"] = 53
    else:
